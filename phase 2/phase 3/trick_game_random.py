...
class Game(object):
    ...
    def play_game(self):
        hand_num = 0
        #while max(list(self.scores.values())) < 30:
        while hand_num <20:
...
if __name__ == '__main__':
    ...
    files = ARGS.player_file.split(";")
    print(files)
    agent_module1 = importlib.import_module(files[0], '.')
    agent_module2 = importlib.import_module(files[1], '.')
    game = Game(agent_module1.Player(), agent_module2.Player())
    game.play_game()
