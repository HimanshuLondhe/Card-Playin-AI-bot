...
class Game(object):
    ...
    def play_game(self):
        ...
        while max(list(self.scores.values())) < 100:
            ...
            deck = Deck()
            deck.shuffle()
            temp_deck = {"Robo_Squad": ['3C', '5C', '8C', '6C', '2S', 'JS', '5S', '3D', 'AD', '4D', 'KD', '5H', 'QD'],"Leonardo": ['6D', '7H', 'KC', '9D', '3H', 'QC', '4H', 'QS', 'KS', '9H', '2C', '9C', 'TC'],
            "Rafael":['6H', '8S', 'JH', 'JD', '4C', '5D', '4S', '8H', 'KH', '2D', 'TD', '2H', '3S'], "Donatello":['AH', 'AC', 'TH', '8D', 'AS', '7D', '7S', 'QH', '9S', 'TS', '6S', 'JC', '7C']}
            #for player in self._players:
            for i, player in enumerate(self._players):
                print("adding for player: ",player.get_name())
                #player.add_cards_to_hand([card.name for card in deck.deal(13)])
                player.add_cards_to_hand(temp_deck[player.get_name()])
                if player.get_name() == "Robo_Squad":
                    lead = i
